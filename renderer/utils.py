...
def sample_front_circle(camera_params, num_frames):
    trajectory = EasyDict(
        {'name': 'front_circle', 'num_frames': num_frames, 'fov_diff': 0.0, 'yaw_diff': 0.25, 'pitch_diff': 0.1,
         'use_mean_camera': True})
    ...
    if trajectory.name == 'front_circle':
        ...
        yaw = camera_params.angles[:, 0].cpu() + trajectory.yaw_diff * torch.sin(
            steps * 2 * np.pi)  # [num_samples * num_frames]
        pitch = camera_params.angles[:, 1].cpu() + trajectory.pitch_diff * torch.cos(
            steps * 2 * np.pi)  # [num_samples * num_frames]
        angles = torch.stack([yaw, pitch, camera_params.angles[:, 2].cpu()], dim=1)  # [num_samples * num_frames, 3]
        fov = (camera_params.fov.cpu() + trajectory.fov_diff * torch.sin(
            steps * 2 * np.pi))  # [num_samples * num_frames]
    elif trajectory.name == 'line':
        yaws = torch.linspace(trajectory.yaw_start, trajectory.yaw_end, num_frames).repeat(
            num_samples)  # [num_samples * num_frames]
        pitches = torch.linspace(trajectory.pitch_start, trajectory.pitch_end, num_frames).repeat(
            num_samples)  # [num_samples * num_frames]
...
class TensorGroup(EasyDict):
    ...
    def __init__(self, **kwargs):
        ...
        assert all((isinstance(t, torch.Tensor) or isinstance(t, TensorGroup)) for t in
                   values), f"Wrong types for values: {dict(zip(keys, [type(v) for v in values]))}"
        assert all(
            len(t) == len(values[0]) for t in values), f"Wrong shapes: {dict(zip(keys, [v.shape for v in values]))}"
        assert all(t.device == values[0].device for t in
                   values), f"All tensor should be on the same device, but got: {dict(zip(keys, [v.device for v in values]))}"
    ...
    def cat(tgroups: List["TensorGroup"], dim: int = 0):
        ...
        assert [set(tg.keys()) == keys_set for tg in
                tgroups], f"Keys should be the same: {[list(tg.keys()) for tg in tgroups]}"
